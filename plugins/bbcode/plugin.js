...
(function () {
  ...
  var bbcodeMap = {
    ...
    // a = attribute, p = parameter (optional)
    videoMap = {
      youtube: {
        height: 'a', width: 'a', src: 'a',
        allowfullscreen: 'p', autoplay: 'p', controls: 'p', nocookie: 'p', rel: 'p', responsive: 'p', start: 'p'
      }
    };
  ...
  CKEDITOR.BBCodeParser.prototype = {
    parse: function (bbcode) {
      ...
      while ((parts = this._.bbcPartsRegex.exec(bbcode))) {
        ...
        // get tag without attributes, e.g. "youtube height"
        if (part.indexOf(' ') > 0) {
          var tagAndAttributes = part.split(' ');
          part = tagAndAttributes[0];
          // remove tag
          tagAndAttributes.splice(0, 1);
          // sanitize attributes
          parts[2] = tagAndAttributes.join(' ') + '=' + parts[2];
        }
        ...
        if (parts[1]) {
          ...
          if (optionPart) {
            ...
            if (part == 'youtube') {
              ...
              attrs = optionPart.split(' ');
              for (var index in attrs) {
                // separate attributes from url parameters
                keyValue = attrs[index];
                temp = keyValue.split('=');
                if (temp.length === 2 && temp[0] in videoMap[part]) {
                  if (videoMap[part][temp[0]] === 'a') {
                    attribs[temp[0]] = temp[1];
                  }
                  else if (videoMap[part][temp[0]] === 'p') {
                    params.push(keyValue);
                  }
                }
                else if (temp.length > 2 && temp[0] in videoMap[part]) {
                  // src contain parameters
                  keyValue = temp[0];
                  temp.splice(0, 1);
                  attribs[keyValue] = temp.join('=');
                }
              }
              ...
              if (!attribs.src) {
                var videoId = this.getContent(part, bbcode);
                if (!!videoId) {
                  ...
                  attribs['data-error'] = 'No link or video id!'
                }
              }
            }
    ...
    getContent: function (tagName, text) {
      var regex = new RegExp('\\[' + tagName + ' [^\\]]+\\]([^\\[]+)\\[\\/' + tagName + '\\]'),
        textElements = text.match(regex);
      return !textElements ? '' : textElements[1];
    }
  };
    ...
    proto: {
      ...
      attribute: function (name, val) {
        ...
        switch (name) {
          ...
          // allowfullscreen, autoplay, controls, nocookie, rel, responsive, start
          case 'ytwidth':
            attrName = ' width=';
            break;
          case 'ytheight':
        ...
        elements: {
          $: function (element) {
            ...
            if (tagName in convertMap)
              ...
              // allowed youtube attributes: height, width, src
              ...
              // allowed youtube parameters:
              // allowfullscreen, autoplay, controls, nocookie, rel, responsive, start
              var allowfullscreen = getParameterByName("allowfullscreen", src);
              if (allowfullscreen !== null) {
                element.attributes.ytallowfullscreen = (allowfullscreen >= 1) ? 1 : 0;
              }

              var autoplay = getParameterByName("autoplay", src);
              if (autoplay !== null) {
                element.attributes.ytautoplay = (autoplay >= 1) ? 1 : 0;
              }

              var controls = getParameterByName("controls", src);
              if (controls !== null) {
                element.attributes.ytcontrols = (controls == 0) ? 0 : 1;
              }

              var nocookie = src.toLocaleLowerCase().indexOf("youtube-nocookie");
              if (nocookie != -1) {
                // ToDo: check if youtube-nocookie.com is in url
                element.attributes.ytnocookie = 1;
              }

              var rel = getParameterByName("rel", src);
              if (rel !== null) {
                element.attributes.ytrel = (rel >= 1) ? 1 : 0;
              }

              var responsive = getParameterByName("responsive", src);
              if (responsive !== null) {
                element.attributes.ytresponsive = (responsive >= 1) ? 1 : 0;
              }

              var time = getParameterByName("start", src);
              if (time !== null) {
